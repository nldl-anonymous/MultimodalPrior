...
class GaussianHead(PixelwiseTaskWithDPT):
    ...
    def __init__(self, net, has_conf=False, local_feat_dim=16, hidden_dim_factor=4., hooks_idx=None, dim_tokens=None,
        ...
        # Gaussian Num Channels = 
            # Offset (2) + 
            # Depths (1) +
        # self.head_position = Mlp(in_features=idim,
        #                         hidden_features=int(hidden_dim_factor * idim),
        #                         out_features=3 * self.patch_size**2)
        self.head_position = PixelwiseTaskWithDPT(
            num_channels=3, feature_dim=feature_dim, last_dim=last_dim, hooks_idx=hooks_idx,
            dim_tokens=dim_tokens, depth_mode=depth_mode, postprocess=postprocess, conf_mode=conf_mode, head_type="depth"
        )
        ...
        gaussian_num_channels = 1 + 3 + 4 + 3 * sh_degree
        self.gaussian_dpt = PixelwiseTaskWithDPT(
            ...
            dim_tokens=dim_tokens, depth_mode=depth_mode, postprocess=postprocess, conf_mode=conf_mode, head_type="img"
        )
    ...
    def forward(self, decout, img_shape, prompt=None):
        # pass through the heads
        # pts3d = self.dpt(decout, image_size=(img_shape[0], img_shape[1]))

        # recover encoder and decoder outputs
        enc_output, dec_output = decout[1][0], decout[1][-1]
        cat_output = torch.cat([enc_output, dec_output], dim=-1)  # concatenate
        H, W = img_shape
        B, S, D = cat_output.shape

        # extract gaussian_features
        gaussian_features = self.gaussian_dpt.dpt(decout[0], image_size=(img_shape[0], img_shape[1]), prompt=prompt)
        ...
        # position_features = self.head_position(cat_output)  # B,S,D
        # position_features = position_features.transpose(-1, -2).view(B, -1, H // self.patch_size, W // self.patch_size)
        # position_features = F.pixel_shuffle(position_features, self.patch_size)  # B,d,H,W
        prompt['img'] = None
        position_features = self.head_position.dpt(decout[1], image_size=(img_shape[0], img_shape[1]), prompt=prompt)

        # post process 3D pts, descriptors and confidences
        out = torch.cat([position_features, gaussian_features], dim=1)
